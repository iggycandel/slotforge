...
const FDEFS=[
  ...
  {key:'freespin',       label:'Free Spins',          group:'Bonus Rounds',          color:'#4ac8f0', screen:'freespin',
   ...
   cfg:[
     ...
     { id:'triggerSymbols', label:'Trigger Symbols', type:'text',   default:'3 Scatters', placeholder:'e.g. 3 Scatters', optional:true },
  ...
  {key:'bonus_pick',     label:'Bonus Pick Game',      group:'Bonus Rounds',          color:'#f0a84c', screen:'bonus_pick',
   ...
   cfg:[
     ...
     { id:'trigger',      label:'Trigger',       type:'text',   default:'3 Bonus', placeholder:'e.g. 3 Bonus', optional:true },
   ]},
  ...
  {key:'sticky_wild',    label:'Sticky Wild',          group:'Wild Mechanics',        color:'#c9d84c', screen:'sticky_wild',
   ...
   cfg:[
     ...
     { id:'accumulates', label:'Accumulates', type:'select', default:'Yes — stickies add up', optional:true,
       options:['Yes — stickies add up','No — one at a time'] },
     { id:'multiplier',  label:'Multiplier',  type:'select', default:'None', optional:true,
       options:['None','2×','Random 2×–5×','Progressive'] },
     { id:'trigger',     label:'Trigger',     type:'select', default:'Any Wild landing', optional:true,
  ...
  {key:'buy_feature',    label:'Buy Feature',          group:'Buy / Ante',            color:'#ef7a7a', screen:null,
   ...
   cfg:[
     ...
     { id:'buysInto', label:'Buys Into',       type:'select', default:'Free Spins', optional:true,
  ...
  {key:'cascade',        label:'Cascade / Avalanche',  group:'Cascades & Reactions',  color:'#4adde8', screen:'cascade',
   ...
   cfg:[
     ...
     { id:'symbolSource',  label:'Symbol Source',  type:'select', default:'From top of reel', optional:true,
  ...
  {key:'megaways',       label:'Megaways™',            group:'Special Mechanics',     color:'#c9a84c', screen:null,
   ...
   cfg:[
     ...
     { id:'licencedFrom',   label:'Licenced From',    type:'text',   default:'Big Time Gaming', placeholder:'Licensor', optional:true },
   ]},
  ...
  {key:'gamble',         label:'Gamble',               group:'Gamble',                color:'#7a8aef', screen:'gamble',
   ...
   cfg:[
     ...
     { id:'maxSteps',   label:'Max Steps',   type:'number', default:5, min:1, max:20, optional:true },
...
function switchWorkspace(ws){
  ...
  try {
    ...
    // Include meta for any workspace the React shell renders full-page
    // (assets, typography) so inputs can pre-fill gameName, theme, etc.
    // Canvas / flow / marketing live inside the iframe and don't need it.
    if(ws === 'assets' || ws === 'typography') msg.meta = collectMeta();
...
function _activateMarketingWorkspace(){
  ...
  s.src = '/editor/marketing.js?v=v144';
  s.onload  = function(){
...
window._sfApplyPayload = function(payload){
  ...
  // (user hasn't run generation yet); we only assign when a spec is
  // actually present so the editor's default P.typographySpec sticks.
  try { if(s.typographySpec && typeof s.typographySpec === 'object') P.typographySpec = s.typographySpec; } catch(e){}
...
window._sfBridge = (function(){
  ...
  function getPayload(){
    return {
      ...
      meta: collectMeta(),
      // Latest typography spec generated by the Typography workspace.
      // Null when the user hasn't run it yet for this project. Schema:
      // see types/typography.ts → TypographySpec.
      typographySpec: (typeof P.typographySpec !== 'undefined') ? P.typographySpec : null,
    };
  }
  ...
  async function getThumbnail(){
    try {
      ...
      try {
        var imgs = Array.prototype.slice.call(gf.querySelectorAll('img'));
  ...
  window.addEventListener('message', function(event){
    ...
    // Persist the latest typography spec from the React workspace into
    // P.typographySpec. getPayload() emits it under payload.typographySpec;
    // _sfApplyPayload restores it on reload so the workspace rehydrates
    // without a fresh API call. spec may be null when the user clears it.
    if(msg.type === 'SF_SAVE_TYPOGRAPHY'){
      try {
        P.typographySpec = msg.spec || null;
        if(typeof markDirty === 'function') markDirty();
      } catch(e) { console.warn('[SF] SF_SAVE_TYPOGRAPHY failed:', e); }
      return;
    }
...
/* ── Screens-list bridge to the React shell ──────────────────────────────
 *  Non-canvas workspaces (Art, Marketing, Logic) live entirely on the
 *  React side, with the iframe shrunk to the menubar strip — so the
 *  iframe-rendered #screen-thumbs-panel isn't visible to those users.
 *  The user wanted that screens picker available from the Art view too:
 *  while they upload assets they want to see / switch between game
 *  screens without having to bounce back to the Flow workspace.
 *
 *  Solution: post a flat screens list to the parent every time the
 *  iframe rebuilds tabs (which already triggers _sfRebuildScreenThumbs).
 *  React renders its own thin column from that data. Clicks come back
 *  in as SF_SET_SCREEN messages → switchScreen on the iframe side.
 *
 *  Same source of truth as the iframe panel (computeTabs()), so what
 *  React shows always matches what Flow shows. No duplicate filtering. */
function _sfPostScreensList(){
  try {
    if(window.top === window) return; // not embedded; nothing to notify
    const tabs = computeTabs();
    const flat = [];
    tabs.forEach(t => {
      if(t.isGroup){
        flat.push({ kind:'group', label:t.label, dot:t.dot });
        (t.children || []).forEach(c => {
          flat.push({
            ...
            dot:     c.dot,
            indent:  1,
            isActive: c.key === P.screen,
          });
        });
      } else if(t.key !== 'project'){
        flat.push({
          kind:    'screen',
          key:     t.key,
          label:   t.label,
          dot:     t.dot,
          indent:  0,
          isActive: t.key === P.screen,
        });
      }
    });
    window.parent.postMessage({
      type:     'SF_SCREENS_LIST',
      screens:  flat,
      viewport: P.viewport,
      activeKey: P.screen,
    }, '*');
  } catch(err){ /* never block the editor on a bridge failure */ }
}

/* Wrap _sfRebuildScreenThumbs once it's been assigned by the panel
 * IIFE so every existing call site (rebuildTabs, switchScreen,
 * setViewport, payload load) gets the React notification for free —
 * we don't have to thread a new helper through five touchpoints. */
(function _wrapScreensRebuildToNotifyParent(){
  function tryWrap(){
    if(typeof window._sfRebuildScreenThumbs !== 'function') return false;
    if(window._sfRebuildScreenThumbs.__wrapped) return true;
    const orig = window._sfRebuildScreenThumbs;
    ...
    window._sfRebuildScreenThumbs = wrapped;
    return true;
  }
  // The panel IIFE runs synchronously as the script parses; tryWrap
  // here picks it up immediately. The retry on next tick is a belt-
  // and-braces guard for any future code that swaps in its own version.
  if(!tryWrap()) setTimeout(tryWrap, 0);
})();

/* Listen for messages from the React shell.
 *  - SF_SET_SCREEN          → switch screens (mirrors click on the
 *                             iframe's #screen-thumbs-panel)
 *  - SF_REQUEST_SCREENS_LIST → re-emit the screens list on demand
 *                             (the React Art workspace asks for one
 *                             on mount in case it loaded after the
 *                             iframe's initial push). */
window.addEventListener('message', function(ev){
  const data = ev && ev.data;
  if(!data) return;
  if(data.type === 'SF_SET_SCREEN' && data.key){
    try { if(typeof switchScreen === 'function') switchScreen(data.key); }
    catch(err){ console.warn('[SF] SF_SET_SCREEN failed', err); }
    return;
  }
  if(data.type === 'SF_REQUEST_SCREENS_LIST'){
    _sfPostScreensList();
    return;
  }
});
